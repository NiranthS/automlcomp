...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run PBT for AutoRL")
    parser.add_argument(
        "--n_instances",
        type=int,
        default=1000,
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="/home/niranth/Desktop/Work/automl_comp/dac4automlcomp/DAC4RL/baselines/pbt/logs/",
        help="Directory where to save trained models and logs.",
    )
    args, _ = parser.parse_known_args()

    logdir = Path(f"{args.outdir}/pbt_seed{args.env_seed}")
    ...
    pbt = PBT(
        ...
        hyperparam_mutations={
            # "algorithm": [0.0, 2.0],
            "learning_rate": lambda : 0.00001*random.randint(1, 10000),
            # "gamma": [0.0001, 1.0],
            "gae_lambda": lambda: random.uniform(0.8, 0.9),
            # "vf_coef": [0.0001, 0.999],
            "ent_coef": lambda: 0.0001 * random.randint(1, 100),
            "clip_range": lambda: random.uniform(0.1, 0.5),
            "buffer_size": lambda: random.randint(1000, 60000 ),
    ...
    for i in range(0, len(json_list)):
        print(i)
        schedule.append(json.loads(json_list[i][:-1]))
        # line = f.readline()
    ...
    hyperparams["algorithm"] = schedule[0]["algorithm"]
